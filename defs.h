...
public:
    ...
    // Cache = Cache.txt + Cache TT (2 x num_cache_TT_leaf_nodes) + Cache TT Buffer        ==> Cache total size = 16 pages = 64 records  
    // RAM = RAM.txt + RAM-Buffer.txt + RAM TT (2 x num_ram_TT_leaf_nodes) + RAM TT Buffer ==> RAM total size = 284 pages = 1136 records
    ...
    static const RowCount num_cache_TT_leaf_nodes = page_size*4; // Cache.txt size = Cache TT Fan-in, 16 records = 4 pages
    static const RowCount num_ram_TT_leaf_nodes = page_size*8; // RAM TT Fan-in = 8 pages = 128 records 
