...
# New test cases for -i parameter with fixed size 1500
# i = 0 => unsorted input data, -1 => desc sort input data, 1 => asc sort input data
NEW_TEST_SIZE=1500
I_VALUES=(0 1 -1)

for I in "${I_VALUES[@]}"; do
    OUTPUT_FILE="$OUTPUT_DIR/test_case_n${NEW_TEST_SIZE}_i${I}.txt"
    echo "Generating test case with -n $NEW_TEST_SIZE and -i $I..."
    ./Test.exe -n "$NEW_TEST_SIZE" -i "$I" > "$OUTPUT_FILE"
    echo "Test case saved to $OUTPUT_FILE"
done
...
# E. Sorted(Asc/Desc) Input -- Scenario is tested and validated to work correctly. Limitation, handles upto 1,00,000 records only